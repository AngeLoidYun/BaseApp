...
public class SelectBarTab extends FrameLayout {
    ...
    public SelectBarTab(Context context, CharSequence title) {
        this(context, null, 0, title);
    }

    public SelectBarTab(Context context, @DrawableRes int icon, CharSequence title) {
        this(context, null, icon, title);
    }

    public SelectBarTab(Context context, AttributeSet attrs, int icon, CharSequence title) {
        this(context, attrs, 0, icon, title);
    }

    public SelectBarTab(Context context, AttributeSet attrs, int defStyleAttr, int icon, CharSequence title) {
...
//            lLContainer.addView(mIcon);
        ...
        mTvTitle.setTextColor(ContextCompat.getColor(context, R.color.blue));
        mTvTitle.setLayoutParams(paramsTv);
        lLContainer.addView(mTvTitle);
        mLinearLayout = lLContainer;
        addView(mLinearLayout);

        int min = dip2px(context, 20);
        int padding = dip2px(context, 5);
        mTvUnreadCount = new TextView(context);
        mTvUnreadCount.setBackgroundResource(R.drawable.maintab_msg_bubble);
...
//            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.white));
            mLinearLayout.setBackgroundColor(ContextCompat.getColor(mContext,R.color.blue));
